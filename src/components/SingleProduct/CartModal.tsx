...
import { decrementProduct, deleteCart, incrementProduct } from '../../features/UserSlice';
...
const CartModal: React.FC = () => {
    ...
    return (
        <>
            <Drawer className='addToCartDrawer' closeIcon={<CloseOutlined style={{ float: 'right' }} />} title="Shopping Cart" placement="right" onClose={onClose} open={open}>
                ...
                <ul className="addToCart">
                    {cartProducts.map((item, index) =>
                        <li key={index} className="addToCartProductItem">
                            <span className='addToCartImg'> <img src={item.image} alt="img" /></span>
                            <span className='addToCartTitle'>
                                ...
                                <div className='addToCartQuantity'>
                                    ...
                                    <span className='btnGroupCart'>
                                        <span className='decrement' onClick={() => dispatch(decrementProduct(item))}>-</span>
                                        <span className='quantityInput'>{item.quantity}</span>
                                        <span className='increment' onClick={() => dispatch(incrementProduct(item))}>+</span>
                                    </span>
                                </div>
                                <span className='deleteCartItem' onClick={() => dispatch(deleteCart(item))}>X</span>
                            </span>
                        </li>
                    )}
                </ul>
                <div className='totalGroup'>
                    <div className="totalPrice">
                        <span className='totalTitle'><h6>Total</h6></span>
                        <span className='totalNumber'>${getTotal().totalPrice}</span>
                    </div>

                    <div className='viewCart'> <a href="_" >view cart</a></div>
                    <div className="checkOut"> <a href="_" >check out</a></div>
                </div>
            </Drawer>
        </>
    )
