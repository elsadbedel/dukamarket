...
const WishList: React.FC = () => {
    ...
    return (
        <>
            <Drawer className='wishListDrawer' closeIcon={<CloseOutlined style={{ float: 'right' }} />} title="Add Wish List" placement="right" onClose={onClosee} open={openn}>
                <ul className="wishList">
                    {wishList.map((item, index) =>
                        <li key={index} className="wishListProductItem">
                            <span className='wishListImg'> <img src={item.image} alt="img" /></span>
                            <span className='wishListTitle'>
                                <h3>{item.title}</h3>
                                <div className='wishListPrice'><p >${item.price}</p></div>
                                <span className='deleteWishList' onClick={() => dispatch(deleteWish(item))}>X</span>
                            </span>
                        </li>
                    )}
                ...
                <div className='totalGroup'>
                    <div className="viewWishList"> <a href="_" >View wishlist</a></div>
                </div>
            </Drawer>
        </>
    )
